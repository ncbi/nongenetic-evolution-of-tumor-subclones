...
##########
### Simulate multiple data replicates under adaptive evolution 
...
if (length(args) < 8) {
  ...
  cat("regime-file: file name containing regime information\n")
  cat("theta-root: expression value at the root\n")
  cat("theta0: expression value for the first regime\n")
  cat("theta1: expression value for the second regime\n")
  cat("sigmasq: the sigma squared value\n")
  cat("r: the r value (dispersion parameter)\n")
  cat("alpha: alpha value\n")
  ...
  cat("regime0: name of first regime\n")
  cat("regime1: name of second regime\n")
	quit(status=1)
}

tree_file = args[1] # File containing newick string
outfile = args[2]   # Output file name
regime_file = args[3] # Regime file
theta_root = as.numeric(args[4])  # State value at the root
theta0 = as.numeric(args[5])  # Optimum for regime 1
theta1 = as.numeric(args[6])  # Optimum for regime 2
sigmasq = as.numeric(args[7])
r = as.numeric(args[8])
alpha = as.numeric(args[9])
ngene = as.integer(args[10])
nrep = as.integer(args[11])
rep_drop = as.numeric(args[12])
regime0 = args[13]
regime1 = args[14]
